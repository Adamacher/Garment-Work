...
function getPurchaseExportRenderOptions(options = {}) {
  ...
  return {
    ...exportOptions,
    isCardLayout,
    sheetWidth: isCardLayout ? 790 : 1080,
    pdfPageSize: isCardLayout ? 'content' : 'A4',
    imageOptions: isCardLayout
      ? { zoomFactor: 2.4, baseWidth: 790, baseHeight: 503, maxWidth: 2600, maxHeight: 6000 }
...
function buildPurchaseOrderHtml(document, options = {}) {
  ...
  if (exportOptions.isCardLayout) {
            ...
            ${hasMaterialImages ? `
              <td class="thumb-cell">
                ${item.materialImageData ? `
                  <div class="thumb-box">
                    <img class="thumb-image" src="${item.materialImageData}" alt="material" />
                  </div>
    ...
    const cardCols = `
      ...
      ${hasMaterialImages ? '<col style="width:54px;" />' : ''}
    ...
    return `
      ...
      <html lang="zh-CN">
        ...
        <body>
          <div class="sheet" data-auto-fit="card" data-fit-width="716">
  ...
  const rows = items
    .map(
      ...
      <tr>
        <td>${item.seqNo}</td>
        ${hasMaterialImages ? `<td class="image-td" style="text-align:center;vertical-align:middle;padding:6px !important;">${item.materialImageData ? `<div class="image-box" style="width:72px;height:72px;max-width:72px;max-height:72px;margin:0 auto;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#f8f6f1;border:1px solid #d7c8b5;border-radius:10px;"><img class="line-image" src="${item.materialImageData}" alt="material" style="display:block;max-width:72px;max-height:72px;width:auto !important;height:auto !important;object-fit:contain;" /></div>` : ''}</td>` : ''}
  ...
  if (!exportOptions.isCardLayout) {
    const a4HeaderCols = `
      ...
      ${hasMaterialImages ? '<col style="width: 96px;" />' : ''}
      <col style="width: 160px;" />
      <col style="width: 80px;" />
      <col style="width: 80px;" />
      <col style="width: 120px;" />
      <col style="width: 110px;" />
      ${showPriceType ? '<col style="width: 100px;" />' : ''}
      ${showUnitPrice ? '<col style="width: 120px;" />' : ''}
      ${showItemRemark ? '<col style="width: 170px;" />' : ''}
    `
  ...
  if (exportOptions.isCardLayout) {
    ...
    return `
      ...
      <html lang="zh-CN">
        ...
        <body>
          <div class="sheet" data-auto-fit="card" data-fit-width="742">
            <div class="fit-surface">
  ...
  const sections = (documents || []).map((document) => {
    ...
    const rows = items
      .map(
        ...
        <tr>
          <td>${item.seqNo}</td>
          ${hasMaterialImages ? `<td class="image-td" style="text-align:center;vertical-align:middle;padding:6px !important;">${item.materialImageData ? `<div class="image-box" style="width:${exportOptions.isCardLayout ? '52px' : '88px'};height:${exportOptions.isCardLayout ? '52px' : '88px'};max-width:${exportOptions.isCardLayout ? '52px' : '88px'};max-height:${exportOptions.isCardLayout ? '52px' : '88px'};margin:0 auto;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#f8f6f1;border:1px solid #d8cfc3;border-radius:10px;"><img class="line-image" src="${item.materialImageData}" alt="material" style="display:block;max-width:${exportOptions.isCardLayout ? '52px' : '88px'};max-height:${exportOptions.isCardLayout ? '52px' : '88px'};width:auto !important;height:auto !important;object-fit:contain;" /></div>` : ''}</td>` : ''}
          <td>${escapeHtml(item.material_code || '')}</td>
          ${showMaterialName ? `<td>${escapeHtml(item.material_name || '-')}</td>` : ''}
          ${showColor ? `<td>${escapeHtml(item.color || '-')}</td>` : ''}
          <td class="number-cell">${formatFixedTrimmed(item.display_qty || item.gross_qty || 0, 2)}</td>
...
ipcMain.handle('order:exportPdf', async (event, orderId) => {
...
ipcMain.handle('order:exportExcel', async (event, orderId) => {
  const { getProductionOrderById } = getDbApi()
  const order = getProductionOrderById(orderId)
  if (!order) throw new Error('生产制单不存在')

  return exportExcelFile(
    buildFactoryOrderHtml(order, { excel: true }),
...
ipcMain.handle('order:exportImage', async (event, orderId) => {
  const { getProductionOrderById } = getDbApi()
  const order = getProductionOrderById(orderId)
  if (!order) throw new Error('生产制单不存在')
...
ipcMain.handle('order:batchExportProductionOrders', async (event, payload = {}) => {
  ...
  for (const id of uniqueIds) {
    ...
    await exportByFormatToPath({
      format,
      html: buildFactoryOrderHtml(order, { excel: format === 'excel' }),
      filePath,
